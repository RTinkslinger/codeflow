...
```
Note: pino 9.x ships its own TypeScript types — `@types/pino` is deprecated and must NOT be installed.
...
function walkImports(node: TreeSitter.SyntaxNode, cb: (mod: string) => void) {
...
function walkDefinitions(node: TreeSitter.SyntaxNode, _source: string, cb: (name: string, kind: 'class' | 'function') => void) {
...
Prerequisites:
- `scip-typescript` binary on PATH: `npm install -g @sourcegraph/scip-typescript`
- `scip` CLI binary on PATH (for protobuf→JSON conversion): `brew install sourcegraph/tap/scip` or `go install github.com/sourcegraph/scip/cmd/scip@latest`
...
Prerequisites:
- `scip-python` binary on PATH: `pip3 install scip-python`
- `scip` CLI binary on PATH (for protobuf→JSON conversion): `brew install sourcegraph/tap/scip` or `go install github.com/sourcegraph/scip/cmd/scip@latest`
Document both in README.
...
```typescript
  async startPreview(opts: { path: string; verified?: boolean }): Promise<{ url: string; previewId: string; status: PreviewStatus }> {
    ...
    this.runFastExtraction(record)
    if (opts.verified) {
      // Verified lane runs in parallel — fire-and-forget (do NOT call runFastExtraction again here)
...
Update the existing `@codeflow/core` import in `mcp.ts` to include `computeDiff`:
```typescript
...
**Systematic debugging pass fixes (2026-04-25):**
- `deduplicateDocuments` circular dep — removed `canonicalizePath` from `core/merger.ts`; added `CFDocument` to import; uses `d.absPath` directly (Task 5)
- `@types/pino` deprecated — removed install command; pino 9 ships own types (Task 4)
- `TreeSitter.SyntaxNode` type — corrected from `Parser.SyntaxNode` in tree-sitter-python walk functions (Task 11)
- `computeDiff` import — clarified as update to existing `@codeflow/core` import line, not a second import (Task 26)
- Verified lane double-invocation — removed spurious `runFastExtraction` call inside `if (opts.verified)` block (Task 26)
- `scip` CLI prerequisite undocumented — added to Task 12 and Task 13 headers with install commands
