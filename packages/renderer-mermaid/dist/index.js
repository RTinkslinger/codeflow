...
function buildSafeIdMap(ir) {
    const idToSafe = new Map();
    const taken = new Set();
    const allIds = [
    ...
    return (id) => idToSafe.get(id) ?? id.replace(SAFE_ID_RE, '_').slice(0, 60);
}
function nodeLabel(sym) {
    return sanitizeLabel(sym.viz?.label ?? sym.name);
}
export function renderMermaid(ir, direction = 'LR') {
