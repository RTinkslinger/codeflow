...
const HAS_SCIP_PY = (() => { try { execSync('scip-python --version', { stdio: 'ignore' }); return true } catch { return false } })()
...
describe.skipIf(!HAS_SCIP_TS || !HAS_SCIP_PY)('mcp mixed-language monorepo (TS + Py)', () => {
  ...
  it('detects TS pkg-a, TS pkg-b, Python pkg-c (setup.py-only) in a single graph', async () => {
    ...
    if (!ir) throw new Error('ir is null')
    ...
    if (!pkgC || !pkgA || !pkgB) throw new Error('one or more workspaces missing from IR')
