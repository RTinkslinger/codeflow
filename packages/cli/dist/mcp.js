    ...
    async runVerifiedExtraction(record) {
        ...
        try {
            ...
            if (irs.some(ir => ir.symbols.length > 0)) {
                ...
                const mermaid = renderMermaid(canonical);
                const allWorkspaceErrors = [];
                if (tsResult.status === 'fulfilled')
                    allWorkspaceErrors.push(...(tsResult.value.workspaceErrors ?? []));
                if (pyResult.status === 'fulfilled')
                    allWorkspaceErrors.push(...(pyResult.value.workspaceErrors ?? []));
                const workspaceWarnings = allWorkspaceErrors.map((e) => ({
