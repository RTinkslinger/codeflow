        ...
        async extract(opts) {
            return {
                ir: {
                    ...
                    documents: [{ relPath: 'a.ts', absPath: path.join(opts.root, 'a.ts'), language: 'ts' }],
                    symbols: [{
    ...
    it('clears verifiedIR so getIR returns inferred symbols after verified re-extraction returns empty', async () => {
        ...
        try {
            ...
            while (Date.now() < deadline1) {
                ...
                if (firstResult.ir?.symbols.some(s => s.confidence === 'verified'))
                    break;
            ...
            while (Date.now() < deadline2) {
                ...
                if (callCount.value >= 2 && secondResult.ir && !secondResult.ir.symbols.some(s => s.confidence === 'verified'))
                    break;
...
//# sourceMappingURL=mcp.stale-verified-ir.test.js.map