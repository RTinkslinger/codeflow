...
function listRecentDiags() {
    ...
    return fs.readdirSync(DIAG_DIR)
        ...
        .map(p => { try {
        return JSON.parse(fs.readFileSync(p, 'utf-8')).code;
    }
    catch {
        return '?';
    } });
}
export async function saveDiagBundle(diagId, contents) {
    evictOldBundles();
    ...
    if (contents.irPartial)
        fs.writeFileSync(path.join(bundleDir, 'ir-partial.json'), JSON.stringify(contents.irPartial, null, 2));
    if (contents.stderrTail)
        fs.writeFileSync(path.join(bundleDir, 'subprocess-stderr.txt'), contents.stderrTail);
