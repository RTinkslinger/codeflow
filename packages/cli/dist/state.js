export class LaneStateMachine {
    _state = 'idle';
    get state() { return this._state; }
    onSave() {
        if (this._state === 'aborted')
            return;
        if (this._state === 'extracting') {
            this._state = 'dirty';
            return;
        }
        if (this._state === 'dirty')
            return; // coalesce: already queued one re-run
        this._state = 'extracting';
    }
    onOk() {
        if (this._state === 'dirty') {
            this._state = 'extracting';
            return;
        } // depth-1 re-run
        if (this._state === 'extracting') {
            this._state = 'idle';
            return;
        }
    }
    onFail() {
        if (this._state === 'extracting' || this._state === 'dirty') {
            this._state = 'error';
        }
    }
    onStop() {
        this._state = 'aborted';
    }
}
export function derivePreviewStatus(laneStates) {
    if (laneStates.every(s => s === 'aborted'))
        return 'aborted';
    if (laneStates.some(s => s === 'aborted'))
        return 'aborted';
    // ALL non-aborted lanes must be in error to surface as error (spec §7 lane-scoped rule)
    const nonAborted = laneStates.filter(s => s !== 'aborted');
    if (nonAborted.length > 0 && nonAborted.every(s => s === 'error'))
        return 'error';
    if (laneStates.some(s => s === 'extracting' || s === 'dirty'))
        return 'extracting';
    // One lane error + other lane idle (has IR) → 'ready' (fast-view is available, verified won't retry)
    return 'ready';
}
//# sourceMappingURL=state.js.map