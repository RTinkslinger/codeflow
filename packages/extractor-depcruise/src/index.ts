 ...
 */
function findDepcruiseBin(): string {
  const candidates = [
  ...
  for (const c of candidates) {
    if (c === 'depcruise') return c  // always accept PATH fallback last
  ...
  async extract(opts: ExtractorOptions): Promise<ExtractorResult> {
    ...
    try {
      const result = await execFileAsync(
        DEPCRUISE_BIN,
        ['--no-config', '--output-type', 'json', opts.path],
    ...
    const parsed: DepCruiseOutput = JSON.parse(jsonStr) as DepCruiseOutput
