...
describe('runPerWorkspace cancellation', () => {
    it('lane budget timeout kills consumer-spawned child processes (no zombies)', async () => {
        ...
        }, { concurrency: 2, timeoutMs: 30_000, laneBudgetMs: 500 });
    ...
    it('per-item timeout kills only that item, others continue', async () => {
        const pids = [];
        const out = await runPerWorkspace(items, async (w, signal) => {
            const child = spawn('sleep', [w.workspaceRel === 'a' ? '60' : '0.05']);
            if (child.pid !== undefined)
                pids.push({ ws: w.workspaceRel, pid: child.pid });
        ...
        }, { concurrency: 2, timeoutMs: 200, laneBudgetMs: 30_000 });
...
//# sourceMappingURL=run-per-workspace-cancel.test.js.map