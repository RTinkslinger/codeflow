 ...
 */
export function reRootIR(ir, repoRoot, workspaceRel) {
    const canonRepoRoot = canonicalizePath(repoRoot);
    const documents = ir.documents.map(d => {
    ...
    const relationships = ir.relationships.map(r => {
        if (r.source && !r.source.file.startsWith('file::')) {
            throw new Error(`reRootIR: Relationship.source.file must be a file-symbol id (starting with "file::"); got "${r.source.file}". This indicates a PR1 contract violation in the upstream extractor.`);
        }
        return r;
    });
    return {
...
//# sourceMappingURL=re-root-ir.js.map