 ...
 */
export function stitchCrossWorkspaceEdges(ir) {
    ...
    const displayNameToWorkspaceRel = new Map();
    ...
    const defTable = new Map(); // key → file-symbol id
    const ambiguous = new Set();
    for (const sym of ir.symbols) {
        if (!sym.workspaceRel)
            continue;
        if (!sym.id.startsWith('file::'))
            continue; // only file-symbols survive canonicalMerge
        ...
        if (defTable.has(key)) {
            if (!ambiguous.has(key)) {
                ambiguous.add(key);
                process.stderr.write(`[codeflow:stitch] ambiguous match for key "${key}": ` +
    ...
    let stitchedCount = 0;
    const rewrittenRels = ir.relationships.map(rel => {
        if (internalSymbolIds.has(rel.to))
            return rel; // already internal
        const parsed = parseScipSymbol(rel.to);
        if (!parsed)
            return rel;
        const { pkgName, descriptor } = parsed;
        if (!displayNameToWorkspaceRel.has(pkgName))
            return rel;
 ...
 */
function parseScipSymbol(id) {
    if (id.startsWith('file::') || id.startsWith('local '))
        return null;
    const parts = id.split(' ');
    if (parts.length < 5)
        return null;
    const [scheme, manager, pkgName, version, ...rest] = parts;
 ...
 */
function extractFileToken(descriptor) {
...
//# sourceMappingURL=cross-workspace-stitch.js.map