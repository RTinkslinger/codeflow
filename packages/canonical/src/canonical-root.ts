...
const MANIFEST_FILES = ['pnpm-workspace.yaml', 'package.json', 'pyproject.toml'] as const
...
export async function resolveCanonicalRoot(startPath: string): Promise<string> {
  ...
  while (true) {
    for (const m of MANIFEST_FILES) {
      ...
      try {
        if (m === 'package.json') {
          ...
          if (
            content.includes('[tool.uv.workspace]') ||
    ...
    if (parent === dir) return start   // filesystem root
    dir = parent
  }
}
